...
nf <- length(unlist(sapply(search()[grep("package",search())],lsf.str)))
library(knitr)
...
## Code
Before we get started, the code for all three lessons has been consolidated into a single R script.  If you'd like, download [`r7_intro_code.R`](https://raw.githubusercontent.com/USEPA/region1_r/master/lessons/r1_intro_code.R) and follow along during the live coding. 
...
The basic syntax of function follows the form: `function_name(arg1, arg2, ...)`.  With the base install, you will gain access to many (`r nf `).  For instance:
