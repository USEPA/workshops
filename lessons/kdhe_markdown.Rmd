...
The purpose of this document is to serve as motivating example (i.e. R, Markdown, and Open data are cool!), but will also serve to structure the rest of this set of lessons on  in that we will see how to work with and visualize data in R, combine code and documentation with R Markdown, and introduce Git and GitHub. Additionally, the tools we will be using come from [The Tidyverse](https://tidyverse.org) which is an opinionated (but effective) way to think about organizing and analyzing data in R.
...
emiss_gdp_gg <- gap_data %>%
  ...
  geom_point(aes(group = country),size = 3) +
  geom_smooth() +
  theme_classic() +
...
![animated gif](https://media.giphy.com/media/eEtxYul1kBS80/giphy.gif)
